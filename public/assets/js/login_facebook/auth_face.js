  ...
  FB.api(
    ...
    function (response) {
      ...
      $.ajax({
        ...
        document
          .getElementById("fbLink1")
          .setAttribute("onclick", "fbLogout()");
      document.getElementById("fbLink1").innerHTML =
        '<div class="d-flex"> <div class="mt-1"> <span class="avatar"> <img src="' +
        response.picture.data.url +
        '" alt="img" class="rounded-circle" style="width: 30px; height: 30px;"></span></div><div class="d-flex1 mt-2">&nbsp; <a>ออกจากระบบ </a></div></div> '; // from Facebook
      document.getElementById("fbLink2").setAttribute("onclick", "fbLogout()");
      document.getElementById("fbLink2").innerHTML =
        '<div class="d-flex"> <div class="mt-1"> <span class="avatar"> <img src="' +
        response.picture.data.url +
        '" alt="img" class="rounded-circle" style="width: 30px; height: 30px;"></span></div><div class="d-flex1 mt-2">&nbsp; <a>ออกจากระบบ </a></div></div> '; // from Facebook

      var list, index;
      list = document.getElementsByClassName("sale-list");
      for (index = 0; index < list.length; ++index) {
        list[index].setAttribute("onclick", "buyTickets(this.id)");
      }
...
function fbLogout() {
  ...
  document.getElementById("fbLink1").innerHTML =
    '<i class="fa fa-facebook-square"  style="font-size:24px" aria-hidden="true"></i> เข้าสู่ระบบด้วย Facebook';
